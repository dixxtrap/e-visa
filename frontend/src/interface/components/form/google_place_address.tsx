...
withLatLng=false
  ...
  const getLocation = () => {
    navigator.geolocation.getCurrentPosition((position) => {
     ...
     if(withLatLng===true) {  form.setFieldValue("latitude", position.coords.latitude);
  ...
  useEffect(() => {
    if(debouncedAddress?.length>2 && !isFirst){const headers: Headers = new Headers();
  ...
  const getLocationById = (id: string, name: string) => {
    ...
    return fetch(`https://places.googleapis.com/v1/places/${id}`, {
      ...
      if(withLatLng===true){ form.setFieldValue("latitude", data.location.latitude);
  ...
  return (
    <>
      ...
      />
      {useMyPosition ? (
        <AppTextInput readOnly value={form.getValues().address} />
      ) : (
        <>
          <AppTextInput
            label="addresse"
